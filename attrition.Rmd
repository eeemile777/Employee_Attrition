...
What Factors Contribute to Employee Attrition, and How Can We Improve Retention?

#Data Description
Employee Attrition Dataset

-Overview:
This dataset contains information related to employees in a company and is designed to predict employee attrition (whether an employee leaves the company or not). The dataset is divided into two parts: a training set and a test set, with a total of 25,491 records in the training set and 4,507 records in the test set.

-Features:

satisfaction_level: A numerical feature representing the employee's reported level of satisfaction. It ranges from 0 to 10.

last_evaluation_rating: A numerical feature representing the employee's last evaluation rating. Ratings range from 0 to 10.

projects_worked_on: A numerical feature indicating the number of projects each employee has worked on. 

average_monthly_hours: A numerical feature representing the average number of monthly hours worked by employees. 

time_spend_company: A numerical feature indicating the number of years an employee has spent with the company. 

Work_accident: A binary feature (0 or 1) indicating whether an employee has had a work accident. 

promotion_last_5years: A binary feature (0 or 1) indicating whether an employee has been promoted in the last 5 years. 

Department: A categorical feature representing the department in which each employee works.

salary: A categorical feature representing the salary level of each employee, with values like "low," "medium," and potentially "high." 

-Target Variable:

Attrition: A binary feature indicating whether an employee has left the company (1 for attrition) or is still employed (0 for no attrition). 
...
This class imbalance can pose challenges in developing predictive models. Machine learning algorithms may tend to favor the majority class, potentially resulting in reduced predictive accuracy for the minority class.
...
```
satisfaction_level: The average satisfaction level among employees is approximately 5.52/10. This suggests that, on average, employees have a moderate level of satisfaction.

last_evaluation_rating: The average last evaluation rating is about 7.17/10. This indicates that, on average, employees tend to have relatively high evaluation ratings.

projects_worked_on: On average, employees have worked on approximately 4.32 projects. This suggests that the average employee is involved in multiple projects.

average_monthly_hours: The average number of average monthly hours worked by employees is roughly 208.08 hours. This indicates that, on average, employees work a moderate number of hours per month.

time_spend_company: On average, employees have spent about 3.64 years with the company. This suggests that the average employee has been with the company for a relatively short to moderate duration.
...
```
Satisfaction Level: Employees who left the company (attrition 1) had significantly lower satisfaction levels (median 4.10) compared to those who stayed (median 6.85). This suggests that lower satisfaction levels are associated with higher attrition rates.

Last Evaluation Rating: Employees who left the company (attrition 1) had slightly higher last evaluation ratings (median 7.90) compared to those who stayed (median 7.20). While this difference is subtle, it suggests that high evaluation ratings alone may not prevent attrition.

Projects Worked On: Both groups of employees, those who stayed and those who left, tended to work on a similar number of projects. This variable doesn't show a strong difference in attrition rates.

Average Monthly Hours: Employees who left the company (attrition 1) worked more hours on average (median 229.00) compared to those who stayed (median 203.00). This indicates that higher average monthly hours might contribute to attrition. It's possible that employees who are overworked or experiencing burnout are more likely to leave.

Time Spend Company: Employees who left the company (attrition 1) had spent slightly more time at the company (median 4.00 years) compared to those who stayed (median 3.00 years). This could imply that employees worked more in the company might be more likely to leave. One hypothesis could be that they have higher salary expectations or are seeking new challenges elsewhere.
...
```
 It appears that employees with low and medium salaries have a higher attrition rate compared to those with high salaries. This information could be valuable for HR and management to assess whether salary levels are a contributing factor to attrition and whether adjustments may be needed to reduce attrition rates, especially in the low and medium salary categories.
...
While there are differences in the total number of employees across departments, attrition rates appear fairly uniform.The attrition rates are quite similar across these departments 
...
```
Work accidents appear to have a minimal influence on attrition. A majority of employees, both those who stayed and those who left the company, had no work accidents. It does not seem to be a significant driver of attrition.


Promotions in the last 5 years also show a limited influence on attrition. Most employees, regardless of attrition status, did not receive promotions during this period. 
...
Standardization or "z-score scaling" can help make variables more directly comparable and can also improve the performance of certain algorithms.
...
By removing one of the dummy columns, we ensure that our model does not suffer from multicollinearity caused by perfect correlation between dummy variables, which can lead to unstable estimates and interpretation issues.
...
Last Evaluation Rating: A higher last evaluation rating is associated with a slightly higher likelihood of attrition.

Projects Worked On: More projects worked on is associated with a significantly lower likelihood of attrition.

Average Monthly Hours: More average monthly hours worked is associated with a slightly higher likelihood of attrition.
...
Work Accident: Having a work accident (work_accident1) is associated with a significantly lower likelihood of attrition.

Promotion in the Last 5 Years: Having received a promotion in the last 5 years (promotion_last_5years1) is associated with a significantly lower likelihood of attrition.
...
Department Sales & Marketing: Belonging to the Sales & Marketing department (department_sales_marketing1) has a very small positive association with attrition.

Department Tech: Belonging to the Tech department (department_tech1) has a very small negative association with attrition.
...
```
The model seems to perform reasonably well, with an accuracy of around 0.77 on the testing dataset. The F1-score, which balances precision and recall, is also around 0.75-0.76.

The metrics on the test set are reasonably close to those on the training set, indicating that logistic regression model is performing well and not suffering from over-fitting. This suggests that your model generalizes effectively to new, unseen data.
...
-Work Accident (coded as 1): Having a work accident seems to make employees less likely to quit.
-Low Salary (coded as 1): Having a lower salary appears to increase the likelihood of an employee quitting.
-Promotion in Last 5 Years (coded as 1): Getting a promotion in the recent past appears to reduce the chances of an employee leaving the company.
-Satisfaction Level: Employees with lower satisfaction levels are more likely to leave the company.
-Time Spent at the Company: Longer-tenured employees are more likely to leave.
...
```
The logistic regression model with a carefully selected subset of features demonstrates that you can achieve a similar level of predictive accuracy while simplifying the model and improving its resilience to noise. This approach showcases the power of feature selection in machine learning, emphasizing the importance of quality over quantity when it comes to features.

One of the most significant benefits is simplicity. A model with fewer features is easier to understand and interpret. With fewer features, there is a lower risk of overfitting the model to the training data. 
...
```
Random Forest model shows outstanding performance on both the training and test datasets. It demonstrates high accuracy, strong agreement with actual values, and excellent sensitivity and specificity. These metrics suggest that the model is robust and capable of making accurate predictions on new data.
...
```
The RandomForest model with a carefully selected subset of features demonstrates that you can achieve a similar level of predictive accuracy while simplifying the model and improving its resilience to noise. This approach showcases the power of feature selection in machine learning, emphasizing the importance of quality over quantity when it comes to features.
...
```
Decision Tree model shows strong performance on both the training and test datasets. It demonstrates high accuracy, strong agreement with actual values, and excellent sensitivity and specificity. These metrics suggest that the model is robust and capable of making accurate predictions on new data
...
```
When "k" is set to 1, the error rate is approximately 0.0350. This means that the KNN model with only one neighbor has a relatively low error rate, indicating that it closely fits the training data. However, it may be sensitive to noise or outliers.

As you increase the value of "k" to 3, 5, 7, and 9, the error rate also increases. This suggests that the model's performance becomes less sensitive to individual data points as you consider more neighbors. However, it's essential to find a balance because an excessively high "k" value might lead to underfitting.
...
```
5-NN model performs well on both the training and test datasets, with high accuracy and balanced sensitivity and specificity. It indicates that the model generalizes effectively to new, unseen data, and it can be considered a robust classifier for predicting attrition.
...
```
Using the subset showed a better accuracy which aligns with what we want
...
Full Models (All Features):

-AUC (Area Under the ROC Curve):
Random Forest achieved the highest AUC (0.9846), indicating excellent discrimination between classes.
Decision Tree and k-NN also performed well with AUC values of 0.9471 and 0.9436, respectively.
Logistic Regression had a lower AUC of 0.8258, suggesting it's less effective at distinguishing between classes.

-Precision and Recall:
Random Forest achieved the highest precision (0.9772) and recall (0.9923), indicating a high true positive rate and low false positive rate.
k-NN had good precision (0.9724) but slightly lower recall (0.9132).
Logistic Regression showed decent precision (0.7826) and recall (0.7563).
Decision Tree also performed well with precision (0.9341) and recall (0.9622).

-Accuracy:
Random Forest had extremely high accuracy on both the training and test datasets, suggesting it's well-fitted and generalizes effectively.
k-NN achieved good accuracy on both sets but slightly lower than Random Forest.
Decision Tree performed well with accuracy close to k-NN.
Logistic Regression showed a modest level of accuracy.


Subset Models:

-AUC (Area Under the ROC Curve):
Random Forest Subset maintained a high AUC (0.9842) even with a reduced feature set.
k-NN Subset improved its AUC (0.9531) compared to the full k-NN model.
Decision Tree Subset retained the same AUC (0.9471) as the full Decision Tree.
Logistic Regression Subset saw a drop in AUC (0.7623) compared to the full Logistic Regression model.

-Precision and Recall:
Random Forest Subset maintained high precision (0.9792) and recall (0.9895) with the reduced feature set.
k-NN Subset also showed good precision (0.9493) and recall (0.9132) with fewer features.
Decision Tree Subset retained the same precision (0.9341) and recall (0.9622) as the full Decision Tree.
Logistic Regression Subset had a slightly improved precision (0.7662) and slightly lower recall (0.7549).

-Accuracy:
Random Forest Subset maintained high accuracy on both the training and test datasets, indicating that feature reduction did not significantly impact its performance.
k-NN Subset showed a slightly improved test accuracy compared to the full k-NN model.
Decision Tree Subset retained similar accuracy to the full Decision Tree model.
Logistic Regression Subset had a similar level of accuracy to the full Logistic Regression model.


Conclusions:
Random Forest Dominates: Random Forest consistently outperforms other models in terms of AUC, precision, recall, and accuracy. It also maintains its high performance even with a reduced feature set.

Subset Models: Feature subset models, particularly Random Forest Subset and k-NN Subset, demonstrate that feature selection can improve model interpretability without sacrificing much in terms of performance. These models are recommended when model interpretability is a priority.

Logistic Regression Trade-off: Logistic Regression, while less complex, sacrifices AUC and precision for interpretability. The subset model provides a more balanced approach for this algorithm.

Decision Trees: Decision Trees perform well in both full and subset models, offering interpretable results and robust performance.
...
Satisfaction Level: (Consistent with the full model)
Job satisfaction remains the most critical factor affecting attrition.

Time Spent in the Company: (Consistent with the full model)
Tenure is still a significant predictor, indicating that longer-term employees are more likely to stay.

Last Evaluation Rating: (Consistent with the full model)
Employee evaluations remain important in predicting attrition.

Work Accident:
Work accidents, though with lower importance, are worth considering.
Employees who have experienced work accidents may be more likely to leave, possibly due to safety concerns.

Salary: (Consistent with the full model)
Salary continues to be a factor, suggesting that fair compensation is essential in retaining talent.

Promotion in the Last 5 Years:
Recent promotions are also influential.
Employees who haven't been promoted in the last 5 years may seek career growth opportunities elsewhere.


Real-World Implications:
Job satisfaction, evaluations, and tenure are consistently vital factors affecting attrition. Companies should prioritize employee engagement, provide regular feedback, and recognize employee contributions to improve satisfaction and retention.
Addressing work-life balance, workload management, and employee safety can help reduce attrition.
Competitive and fair compensation structures and opportunities for career growth are essential for retaining talent.
These insights can guide HR and management in implementing strategies to reduce attrition and improve the overall employee experience.
In summary, these influential variables provide actionable insights for organizations aiming to reduce attrition and create a more engaging and satisfying work environment for their employees.
...
```
### Interpretation
PC1: Employees with high values on PC1 are likely to exhibit strong work commitment, as indicated by their involvement in more projects, higher average monthly hours, and high evaluation ratings.

PC2: Employees with high values on PC2 are associated with departmental variation, particularly between the "sales/marketing" and "tech" departments. This component highlights differences in employee characteristics or behaviors based on their department.

PC3: Employees with high values on PC3 tend to have higher job satisfaction, have been promoted in the last 5 years, and receive higher salaries. The presence of "work_accident" in this component suggests that employees with accidents might have better scores on this component, possibly due to compensation or attention.

PC4: Employees with high values on PC4 have been promoted and potentially received higher salaries. This component might represent career progression and the influence of promotions and salary on employee profiles. Some of them had work accident which affects satisfaction.

PC5: Employees with high values on PC5 are more likely to have work accidents but less likely to have high satisfaction levels. This component appears to reflect a trade-off between safety and job satisfaction.
...
custom_colors <- c("firebrick3", "#00AFBB", "#E7B800")
...
(Medium Satisfaction, Low Evaluation, Low Workload, Low Tenure, Medium Salary, Medium Attrition):
This cluster represents employees with moderate levels of satisfaction, low evaluation ratings, and lower workloads.
They are relatively new to the company and have medium salaries and attrition rates.
This cluster might represent a mix of junior or new employees who haven't yet reached high workloads or evaluation ratings.

Advice: Focus on Onboarding and Skill Development
Enhance the onboarding process for new employees to ensure a smooth transition.
...
(Low Satisfaction, High Evaluation, High Workload, High Tenure, Low Salary, High Attrition):
This cluster represents a group of employees with high workloads and evaluation ratings but low job satisfaction.
They are relatively long-tenured, but their low salaries and high attrition rates indicate a risk of turnover.
These employees might be dissatisfied due to the salary factor.

Advice: Address Job Satisfaction and Compensation
...
Provide opportunities for skill development and career progression to retain valuable employees.


Cluster 3

(High Satisfaction, High Evaluation, High Workload, High Tenure, High Salary, Low Attrition):
This cluster represents highly engaged and satisfied employees who are dedicated to their work.
They tend to be long-tenured employees with high salaries, indicating loyalty and compensation.
The low attrition rate suggests that these employees are likely to stay with the company.

Advice: Maintain and Nurture Talent
